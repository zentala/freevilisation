...
export function evaluateFrameBudget(frameDurationsMs: readonly number[], minimumFps = 30): FrameBudgetResult {
  ...
  return { frames: fps.length, averageFps, minimumFps: Math.min(...fps), passed: Math.min(...fps) >= minimumFps };
}
